...
trait LogsActivity
{
    ...
    public function logActivity(string $event, ?string $customDescription = null): void
    {
        ...
        $userId = auth()->id();
        // Ensure the user actually exists to avoid foreign key constraint violations
        // This handles cases where a session might contain a deleted user ID
        if ($userId && ! \App\Models\User::where('id', $userId)->exists()) {
            $userId = null;
            // Optionally force logout if the user is invalid, but safe to just log as guest here.
            // auth()->logout();
        }
