...
const form = useForm({
    ...
    discount_price: tutorial.discount_price ?? null,
    is_free: tutorial.is_free ?? false,
    status: tutorial.status ?? 'draft',
    thumbnail: tutorial.thumbnail ?? '',
    has_timeline: tutorial.has_timeline ?? false,
});

const submit = () => {
    form.put(admin.tutorials.update.url(tutorial.id), { onSuccess: () => form.reset() });
};
</script>
    ...
    <AdminLayout>
        <div class="space-y-6">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Tutorial</h1>
            </div>
