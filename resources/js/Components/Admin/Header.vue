...
// Branding helper
const t = (key: string) => {
    if (key === 'title') return page.props.auth.user.name || page.props.branding?.admin?.header?.[key] || 'Admin';
...
<template>
    ...
    >
        ...
        <div class="relative flex items-center gap-2 md:gap-4">
            <NotificationBell
                :recent-url="admin.notifications.recent.url()"
                :view-all-url="admin.notifications.index.url()"
                :mark-as-read-url="(id) => admin.notifications.read.url({ id })"
                :mark-all-as-read-url="admin.notifications.read_all.url()"
            />
